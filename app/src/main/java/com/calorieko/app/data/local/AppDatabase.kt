...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Migration 27 → 28: Adds `scaled_servings` and `tweaks_json` columns
         * to PLANNED_MEALS_TABLE.
         *
         * Persists recipe scaling (serving count override) and individual
         * ingredient tweaking (per-ingredient multipliers) alongside the
         * existing substitution data.
         */
        val MIGRATION_27_28 = object : Migration(27, 28) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "ALTER TABLE PLANNED_MEALS_TABLE ADD COLUMN scaled_servings INTEGER NOT NULL DEFAULT 0"
                )
                db.execSQL(
                    "ALTER TABLE PLANNED_MEALS_TABLE ADD COLUMN tweaks_json TEXT NOT NULL DEFAULT ''"
                )
            }
        }
