...
fun LogMealScreen(onBack: () -> Unit, onMealConfirmed: () -> Unit) {
    ...
    // Stable prediction tracking: require 3s of consistent top-1 label
    ...
    //   • supported dish → wait for 3s of consistent prediction, then → DISH_READY
    LaunchedEffect(weightStable, topLabel, topConfidence, phase, stableSince) {
        if (phase == LogMealPhase.SCANNING && weightStable && weight > 0) {
            if (topLabel == "negative" && topConfidence >= CONFIDENCE_THRESHOLD) {
                ...
                // Require 3 seconds of consistent top-1 prediction
                val elapsed = System.currentTimeMillis() - stableSince
                if (elapsed < 3000) return@LaunchedEffect
        ...
        CameraPreview(
            ...
            onFrameAnalyzed = { results ->
                if (results.isNotEmpty() && phase == LogMealPhase.SCANNING) {
                    ...
                    // Track label stability for the 3-second consistency check
